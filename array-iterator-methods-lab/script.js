...
const isOldInventor = (inventor) => {
    if (inventor.year >= 1500 && inventor.year < 1600) {
        return inventor;
    }
}

veryOldInventors = inventors.filter(isOldInventor);
...
const getNames = (inventor) => {
    return { first: inventor.first, last: inventor.last };
}

inventorNames = inventors.map(getNames);
...
sortedByBirthYear = inventors.sort(sortByYear);
...
const findName = (inventor) => {
    return inventor.first === 'Ada';
}

inventorNamedAda = inventors.find(findName);
...
const formatNames = (person) => {
    const newName = person.split(', ');
    return `${newName[1]} ${newName[0]}`;
}

firstLast = people.map(formatNames);
...
const findId = (comment) => {
    return comment.id === 823423;
}

commentById = comments.find(findId);
...
// could also alter & reuse the function above, but for pupose of this exercise I have created a new function findIdx()
const findIdx = (comment) => {
    return comment.id === 123523;
}

idx = comments.findIndex(findIdx);
...
const calculateYearsLived = (yearsLived, inventor) => {
    return yearsLived + (inventor.passed - inventor.year);
}

totalYearsLived = inventors.reduce(calculateYearsLived, 0);
...
const tallyTravelMethods = (tally, method) => {
    if (tally[method]) {
        tally[method] += 1;
    } else {
        tally[method] = 1;
...
travelMethodCounts = travelMethods.reduce(tallyTravelMethods, {});
