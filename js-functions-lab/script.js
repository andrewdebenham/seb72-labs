...
const convertTemperature = (temp, scale) => {
  if (scale === 'C') {
    const convertedC = ((9 / 5) * temp) + 32;
    return `${ convertedC } (Fahrenheit).`;
  } else {
    const convertedF = (temp - 32) * (5 / 9);
    return `${ convertedF } (Celsius).`
  }
}
...
const createUsername = (first, last) => {
  let username = first[0] + first[1] + first[2] + last[0] + last[1] + last[2];
